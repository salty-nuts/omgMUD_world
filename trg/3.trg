...
wait 5
if (%actor.name% != jack)
%send% %actor% The axe says, 'Goodbye.'
%purge% self
else
%send% %actor% The axe says, 'Greetings, Master'
end
~
#340
jack's restore~
...
if %self.worn_by%
  ...
  %damage% %actor% -%hp%
  %echo% %actor.name% is surrounded with a healing aura.
  while %actor.mana% < %actor.maxmana%
    nop %actor.mana(1000)%
    %echo% %actor.name% is surrounded with a magical aura.
  done
end
~
#380

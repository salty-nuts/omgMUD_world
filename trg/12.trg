...
~
#1222
Purge Items in room on Zone Reset~
2 f 100
~
%purge%
~
#1230
salty 100 mana restore~
1 b 100
~
if %self.worn_by%
...
~
#1232
salty magical maxhitp gain~
1 b 100
~
if %self.worn_by%
  set actor %self.worn_by%
  eval roll %random.10%
  nop %actor.maxhitp(%roll%)%
  %echo% %actor.name% looks amazed to be growing magical hitpoints!
end
~
#1233
salty magical maxmana gain~
1 b 100
~
if %self.worn_by%
  set actor %self.worn_by%
  eval roll %random.10%
  nop %actor.maxmana(%roll%)%
  %echo% %actor.name% looks amazed to be growing magical mana!
end
~
#1234
while loop example~
...
if %self.worn_by%
  set actor %self.worn_by%
  if %actor.room.vnum% >= 1800 && %actor.room.vnum% <= 1899
    switch %random.5%
      case 1
        %load% mob 1810
        %echo% a series 800 terminator leaps into existence!
      break
      case 2
        %load% mob 1811
        %echo% a aerial hunter killer leaps into existence!
      break
      case 3
        %load% mob 1813
        %echo% a centurion leaps into existence!
      break
      case 4
        %load% mob 1816
        %echo% the T1000 leaps into existence!
      break
      case 5
        %load% mob 1820
        %echo% a guerilla officer leaps into existence!
      break
      default
        %load% mob 1809
        %echo% John Connor leaps into existence!
      break
    done
  end
end
...
if %self.worn_by%
  set actor %self.worn_by%
  eval hp %actor.maxhitp%
  %damage% %actor% -%hp%
  while %actor.mana% < %actor.maxmana%
    nop %actor.mana(1000)%
  done
end
~
#1237
salty object damage spec~
1 b 100
~
* by Salty
* Written for Frost, example of object damage trigger.
* Including all comments for verbosity.
*
* Since this is an object trigger, self is the object.
* We only execute this trigger if self is equipped by a PC or NPC.
if %self.worn_by% 
  * actor is undefined currently, so we assign it to the value of who is wearing self.
  eval actor %self.worn_by%
  * lowdam is our minimum damage for the spec.
  set lowdam 500
  * highdam is our maximum damage for the spec.
  set highdam 1500
  * range is the difference between highdam and lowdam.
  eval range %highdam% - %lowdam%
  * dam is a random number from zero to range, plus lowdam.
  eval dam (%%random.%range%%% + %lowdam%)
  * This trigger will only cause damage if actor is fighting.
  if %actor.fighting%
    * victim is undefined currently, so we assign it to the value of who the actor is fighting.
    eval victim %actor.fighting%
    * this check prevents the trigger from damaging a player
    if !%victim.is_pc%
      %damage% %victim% %dam%
      %send% %actor% You reach out and touch the soul of %victim.name%! [%dam%]
      %echoaround% %actor% %actor.name% reaches out and touches the soul of %victim.name%.
    end
  end
end
~
